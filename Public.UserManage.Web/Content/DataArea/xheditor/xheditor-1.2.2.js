!function() {
	...
	a.mapLang = {}, a.setLang = function(c, d) {
		d === b && (d = c, c = ""), "" !== c && (c += ".");
		var e, f;
		for (var g in d) f = d[g], e = c + g, "string" == typeof f || f instanceof Array ? a.mapLang[e] = f : a.setLang(e, f)
	}, a.getLang = function(c) {
		var d = a.mapLang[c];
		if (d === b && (d = c.replace(/{#([^{}]+)}/g, function(b, c) {
			var d = a.mapLang[c];
			return d ? d : c
		})), "string" == typeof d) {
			var e = arguments;
			d = d.replace(/{\$(\d+)}/g, function(a, c) {
	...
	if (!a.browser) {
		var d = c(navigator.userAgent),
			e = {};
		d.browser && (e[d.browser] = !0, e.version = d.version), e.chrome ? e.webkit = !0 : e.webkit && (e.safari = !0), a.browser = e
	}
}(jQuery), function(XHEDITOR, $, undefined) {
	var agent = navigator.userAgent.toLowerCase(),
		...
		bShowModal = !1,
		bCheckEscInit = !1,
		_jPanel, _jShadow, _jCntLine, _jPanelButton, jModal, jModalShadow, layerShadow, jOverlay, jHideSelect, onModalRemove, editorRoot, getLang = XHEDITOR.getLang;
		...
		Xheditor = function(textarea, options) {
			...
			function cleanPaste(a) {
				...
				if (a && (b = a.originalEvent.clipboardData) && (c = b.items) && (d = c[0]) && "file" == d.kind && d.type.match(/^image\//i)) {
					...
					return f.onload = function() {
						...
						a = replaceRemoteImg(a), _this.pasteHTML(a)
					}, f.readAsDataURL(e), !1
				}
				...
				var h = isIE ? "pre" : "div",
					...
					k = _this.getSel(),
					l = _this.getRng(!0);
				i.css("top", _jWin.scrollTop()), isIE || isIE11 ? (l.moveToElementText(j), l.select()) : (l.selectNodeContents(j), k.removeAllRanges(), k.addRange(l)), setTimeout(function() {
					var a, b = 3 === g;
					...
					i.remove(), _this.loadBookmark(), a = a.replace(/^[\s\uFEFF]+|[\s\uFEFF]+$/g, ""), a && (b ? _this.pasteText(a) : (a = _this.cleanHTML(a), a = _this.cleanWord(a), a = _this.formatXHTML(a), (!settings.onPaste || settings.onPaste && (a = settings.onPaste(a)) !== !1) && (a = replaceRemoteImg(a), _this.pasteHTML(a)))), bCleanPaste = !1
				}, 0)
			}
			function replaceRemoteImg(a) {
				...
				if (b && c) {
					var d = [],
						e = 0;
					a = a.replace(/(<img)((?:\s+[^>]*?)?(?:\s+src="\s*([^"]+)\s*")(?: [^>]*)?)(\/?>)/gi, function(a, c, f, g, h) {
						return !/^(https?|data:image)/i.test(g) || /_xhe_temp/.test(f) || b.test(g) || (d[e] = g, f = f.replace(/\s+(width|height)="[^"]*"/gi, "").replace(/\s+src="[^"]*"/gi, ' src="' + skinPath + 'img/waiting.gif" remoteimg="' + e+++'"')), c + f + h
					}), d.length > 0 && $.post(c, {
						urls: d.join("|")
					}, function(a) {
						a = a.split("|"), $("img[remoteimg]", _this.doc).each(function() {
							var b = $(this);
							xheAttr(b, "src", a[b.attr("remoteimg")]), b.removeAttr("remoteimg")
						})
					})
				}
				return a
			...
			function fixAppleSel(a) {
				if (a = a.target, a.tagName.match(/(img|embed)/i)) {
					var b = _this.getSel(),
						c = _this.getRng(!0);
					c.selectNode(a), b.removeAllRanges(), b.addRange(c)
				}
			}
			function xheAttr(a, b, c) {
				if (!b) return !1;
				var d = "_xhe_" + b;
				return c && (urlType && (c = getLocalUrl(c, urlType, urlBase)), a.attr(b, urlBase ? getLocalUrl(c, "abs", urlBase) : c).removeAttr(d).attr(d, c)), a.attr(d) || a.attr(b)
			}
			...
			function checkShortcuts(a) {
				...
				sKey = "", sKey += a.ctrlKey ? "ctrl+" : "", sKey += a.altKey ? "alt+" : "", sKey += a.shiftKey ? "shift+" : "", sKey += d;
				var e, f = arrShortCuts[sKey];
				for (e in f) {
					if (e = f[e], !$.isFunction(e)) return _this.exec(e), !1;
			...
			function getLocalUrl(a, b, c) {
				...
				if ("" === h && (h = "80"), "" === i ? i = "/" : "/" !== i.charAt(0) && (i = "/" + i), a = $.trim(a), "abs" !== b && (a = a.replace(new RegExp(e + "\\/\\/" + g.replace(/\./g, "\\.") + "(?::" + h + ")" + ("80" === h ? "?" : "") + "(/|$)", "i"), "/")), "rel" === b && (a = a.replace(new RegExp("^" + i.replace(/([\/\.\+\[\]\(\)])/g, "\\$1"), "i"), "")), "rel" !== b && (a.match(/^(https?:\/\/|\/)/i) || (a = i + a), "/" === a.charAt(0))) {
					var j, k, l = [],
						m = a.split("/"),
						n = m.length;
					for (k = 0; n > k; k++) j = m[k], ".." === j ? l.pop() : "" !== j && "." !== j && l.push(j);
					"" === m[n - 1] && l.push(""), a = "/" + l.join("/")
				}
				return "abs" !== b || a.match(/^https?:\/\//i) || (a = e + "//" + f + a), a = a.replace(/(https?:\/\/[^:\/?#]+):80(\/|$)/i, "$1$2")
			}
			...
			var _this = this,
				...
				_jTools, _jArea, _win, _jWin, _doc, _jDoc, bookmark, bInit = !1,
				bSource = !1,
				bFullscreen = !1,
				bCleanPaste = !1,
				outerScroll, bShowBlocktag = !1,
				sLayoutStyle = "",
				ev = null,
				timer, bDisableHoverExec = !1,
				bQuickHoverExec = !1,
				lastPoint = null,
				lastAngle = null,
				editorHeight = 0,
				settings = _this.settings = $.extend({}, XHEDITOR.settings, options),
				plugins = settings.plugins,
				strPlugins = [];
			if (plugins && (arrTools = $.extend({}, arrTools, plugins), $.each(plugins, function(a) {
				strPlugins.push(a)
			...
			this.init = function() {
				function a(a) {
					var b, c = $(a.target);
					(b = c.css("width")) && c.css("width", "").attr("width", b.replace(/[^0-9%]+/g, "")), (b = c.css("height")) && c.css("height", "").attr("height", b.replace(/[^0-9%]+/g, ""))
				}
				...
				g && (l += "<style>html{background:" + g + ";}</style>"), l += '</head><body spellcheck="0" class="editMode' + bodyClass + '"></body></html>', _this.win = _win = $("#" + idIframe)[0].contentWindow, _jWin = $(_win);
				try {
					this.doc = _doc = _win.document, _jDoc = $(_doc), _doc.open(), _doc.write(getLang(l)), _doc.close(), isIE ? _doc.body.contentEditable = "true" : _doc.designMode = "On"
				} catch (m) {}
				setTimeout(setOpts, 300), _this.setSource(), _win.setInterval = null, _jTools.append(getLang(h.join(""))).bind("mousedown contextmenu", returnFalse).click(function(a) {
					var b = $(a.target).closest("a");
					return b.is(".xheEnabled") && (clearTimeout(timer), _jTools.find("a").attr("tabindex", "-1"), ev = a, _this.exec(b.attr("cmd"))), !1
					...
					if (d && d > 10) return bDisableHoverExec = !0, setTimeout(function() {
						bDisableHoverExec = !1
					}, 100), !1;
					var e = b.attr("cmd"),
						f = 1 === arrTools[e].h;
				...
				}), _jPanel = $("#xhePanel"), _jShadow = $("#xheShadow"), _jCntLine = $("#xheCntLine"), 0 === _jPanel.length && (_jPanel = $('<div id="xhePanel"></div>').mousedown(function(a) {
					a.stopPropagation()
				...
				}), isWebkit && _jWin.click(fixAppleSel), _jDoc.mousedown(clickCancelPanel).keydown(checkShortcuts).keypress(forcePtag).dblclick(checkDblClick).bind("mousedown click", function(a) {
					_jText.trigger(a.type)
				}), isIE && (_jDoc.keydown(function(a) {
					var b = _this.getRng();
					return 8 === a.which && b.item ? ($(b.item(0)).remove(), !1) : void 0
				}), _jDoc.bind("controlselect", function(b) {
					b = b.target, $.nodeName(b, "IMG") && $(b).unbind("resizeend", a).bind("resizeend", a)
					...
					var b = a.which;
					if (27 == b) _jTools.find("a").attr("tabindex", "-1"), _this.focus();
					else if (a.altKey && b >= 49 && 57 >= b) return _jTools.find(".xheGStart").eq(b - 49).next().find("a").focus(), !1
				});
				var n = $(_doc.documentElement);
				isOpera ? n.bind("keydown", function(a) {
					a.ctrlKey && 86 === a.which && cleanPaste()
				}) : n.bind(isIE ? "beforepaste" : "paste", cleanPaste), settings.disableContextmenu && n.bind("contextmenu", returnFalse), settings.html5Upload && n.bind("dragenter dragover", function(a) {
					var b;
					...
					if (d && (c = d.files) && c.length > 0) {
						var e, f, g, h = ["Link", "Img", "Flash", "Media"],
							i = [];
						for (e in h) f = h[e], settings["up" + f + "Url"] && settings["up" + f + "Url"].match(/^[^!].*/i) && i.push(f + ":," + settings["up" + f + "Ext"]);
						return 0 === i.length ? !1 : (g = i.join(","), f = b(c), 1 === f ? alert(getLang("upload.extLimit", g.replace(/\w+:,/g, ""))) : 2 === f ? alert(getLang("upload.typeLimit")) : f && _this.startUpload(c, settings["up" + f + "Url"], "*", function(a) {
							var b, c = [],
								d = settings.onUpload;
							d && d(a);
							for (var e = 0, g = a.length; g > e; e++) b = a[e], url = is(b, "string") ? b : b.url, "!" === url.substr(0, 1) && (url = url.substr(1)), c.push(url);
							_this.exec(f), $("#xhe" + f + "Url").val(c.join(" ")), $("#xheSave").click()
						}), !1)
					}
				});
				var o = settings.shortcuts;
				o && $.each(o, function(a, b) {
				...
				if (!bSource) {
					_this.focus();
					var a = _this.getRng();
					a = a.cloneRange ? a.cloneRange() : a, bookmark = {
						top: _jWin.scrollTop(),
						rng: a
					}
				}
				...
				var b, c;
				try {
					a || (b = _this.getSel(), c = b.createRange ? b.createRange() : b.rangeCount > 0 ? b.getRangeAt(0) : null), c || (c = _doc.body.createTextRange ? _doc.body.createTextRange() : _doc.createRange())
				} catch (d) {}
				return c
			}, this.getParent = function(a) {
				var b, c = _this.getRng();
				return isIE ? b = c.item ? c.item(0) : c.parentElement() : (b = c.commonAncestorContainer, c.collapsed || c.startContainer === c.endContainer && c.startOffset - c.endOffset < 2 && c.startContainer.hasChildNodes() && (b = c.startContainer.childNodes[c.startOffset])), a = a ? a : "*", b = $(b), b.is(a) || (b = $(b).closest(a)), b
				...
				if (c.cloneContents) {
					var f, g = $("<div></div>");
					f = c.cloneContents(), f && g.append(f), e = g.html()
				} else e = is(c.item) ? c.item(0).outerHTML : is(c.htmlText) ? c.htmlText : c.toString();
				return d && (e = ""), e = _this.processHTML(e, "read"), e = _this.cleanHTML(e), e = _this.formatXHTML(e)
			}, this.pasteHTML = function(a, b) {
				if (bSource) return !1;
				_this.focus(), a = _this.processHTML(a, "write");
				var c = _this.getSel(),
					d = _this.getRng();
				if (b !== undefined) {
					if (d.item) {
						var e = d.item(0);
						d = _this.getRng(!0), d.moveToElementText(e), d.select()
					}
					d.collapse(b)
				}
				if (a += "<" + (isIE ? "img" : "span") + ' id="_xhe_temp" width="0" height="0" />', d.insertNode) {
					if ($(d.startContainer).closest("style,script").length > 0) return !1;
					d.deleteContents(), d.insertNode(d.createContextualFragment(a))
				} else "control" === c.type.toLowerCase() && (c.clear(), d = _this.getRng()), d.pasteHTML(a);
				var f = $("#_xhe_temp", _doc),
					g = f[0];
				isIE ? (d.moveToElementText(g), d.select()) : (d.selectNode(g), c.removeAllRanges(), c.addRange(d)), f.remove()
			}, this.pasteText = function(a, b) {
				a || (a = ""), a = _this.domEncode(a), a = a.replace(/\r?\n/g, "<br />"), _this.pasteHTML(a, b)
				...
				return a.replace(regEntities, function(a) {
                ...
                bSource ? $("#sourceCode", _doc).val(a) : (settings.beforeSetSource && (a = settings.beforeSetSource(a)),
                //a = _this.cleanHTML(a),
                //a = _this.formatXHTML(a),
                //a = _this.processHTML(a, "write"),  by wangliang
                isIE ? (_doc.body.innerHTML = '<img id="_xhe_temp" width="0" height="0" />' + a, $("#_xhe_temp", _doc).remove()) : _doc.body.innerHTML = a)
				...
				function c(a, b, c, d, e, g) {
					...
					if (k = d.match(/(?:^|[\s;])color\s*:\s*([^;"]+)/i)) {
						...
						k[1] = k[1].replace(/^#([0-9a-f])([0-9a-f])([0-9a-f])$/i, "#$1$1$2$2$3$3"), l += ' color="' + k[1] + '"'
					}
					return d = d.replace(/(^|;)(\s*(font-family|color)\s*:\s*[^;"]+;?)+/gi, "$1"), "" !== l ? (d && (l += ' style="' + d + '"'), "<font" + (c ? c : "") + l + (e ? e : "") + ">" + g + "</font>") : a
				}
				function d(a, b, c, d, e) {
					for (var f, h, i = (c ? c : "") + (d ? d : ""), j = [], k = [], l = 0; l < g.length; l++) f = g[l].r, h = g[l].t, i = i.replace(f, function() {
						return j.push("<" + h + ">"), k.push("</" + h + ">"), ""
					});
					return i = i.replace(/\s+style\s*=\s*"\s*"/i, ""), (i ? "<span" + i + ">" : "") + j.join("") + e + k.join("") + (i ? "</span>" : "")
				}
				var e = ' class="Apple-style-span"',
					f = settings.listFontsize;
				if ("write" === b) a = a.replace(/(<(\/?)(\w+))((?:\s+[\w\-:]+\s*=\s*(?:"[^"]*"|'[^']*'|[^>\s]+))*)\s*((\/?)>)/g, function(a, b, c, d, f, g, h) {
					d = d.toLowerCase(), isMozilla ? "strong" === d ? d = "b" : "em" === d && (d = "i") : isWebkit && ("strong" === d ? (d = "span", c || (f += e + ' style="font-weight: bold;"')) : "em" === d ? (d = "span", c || (f += e + ' style="font-style: italic;"')) : "u" === d ? (d = "span", c || (f += e + ' style="text-decoration: underline;"')) : "strike" === d && (d = "span", c || (f += e + ' style="text-decoration: line-through;"')));
					var i, j = "";
					if ("del" === d) d = "strike";
					else if ("img" === d) f = f.replace(/\s+emot\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/i, function(a, b) {
						return i = b.match(/^(["']?)(.*)\1/)[2], i = i.split(","), i[1] || (i[1] = i[0], i[0] = ""), "default" === i[0] && (i[0] = ""), settings.emotMark ? a : ""
					});
					else if ("a" === d)!f.match(/ href=[^ ]/i) && f.match(/ name=[^ ]/i) && (j += " xhe-anchor"), h && (g = "></a>");
					else if ("table" === d && !c) {
						var k = f.match(/\s+border\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/i);
						(!k || k[1].match(/^(["']?)\s*0\s*\1$/)) && (j += " xhe-border")
					}
					var l;
					if (f = f.replace(/\s+([\w\-:]+)\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/g, function(a, b, c) {
						return b = b.toLowerCase(), c = c.match(/^(["']?)(.*)\1/)[2], aft = "", isIE && b.match(/^(disabled|checked|readonly|selected)$/) && c.match(/^(false|0)$/i) ? "" : "img" === d && i && "src" === b ? "" : (b.match(/^(src|href)$/) && (aft = " _xhe_" + b + '="' + c + '"', urlBase && (c = getLocalUrl(c, "abs", urlBase))), j && "class" === b && (c += " " + j, j = ""), isWebkit && "style" === b && "span" === d && c.match(/(^|;)\s*(font-family|font-size|color|background-color)\s*:\s*[^;]+\s*(;|$)/i) && (l = !0), " " + b + '="' + c + '"' + aft)
					}), i) {
						var m = emotPath + (i[0] ? i[0] : "default") + "/" + i[1] + ".gif";
						f += ' src="' + m + '" _xhe_src="' + m + '"'
					}
					return l && (f += e), j && (f += ' class="' + j + '"'), "<" + c + d + f + g
				}), isIE && (a = a.replace(/&apos;/gi, "&#39;")), isWebkit || (a = a.replace(/<(span)(\s+[^>]*?)?\s+style\s*=\s*"((?:[^"]*?;)?\s*(?:font-family|font-size|color)\s*:[^"]*)"( [^>]*)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/gi, c), a = a.replace(/<(span)(\s+[^>]*?)?\s+style\s*=\s*"((?:[^"]*?;)?\s*(?:font-family|font-size|color)\s*:[^"]*)"( [^>]*)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?)<\/\1>/gi, c), a = a.replace(/<(span)(\s+[^>]*?)?\s+style\s*=\s*"((?:[^"]*?;)?\s*(?:font-family|font-size|color)\s*:[^"]*)"( [^>]*)?>(((?!<\1(\s+[^>]*?)?>)[\s\S])*?)<\/\1>/gi, c)), a = a.replace(/<(td|th)(\s+[^>]*?)?>(\s|&nbsp;)*<\/\1>/gi, "<$1$2>" + (isIE ? "" : "<br />") + "</$1>");
				else {
					if (isWebkit) for (var g = [{
						...
						t: "strike"
					}], h = 0; 2 > h; h++) a = a.replace(/<(span)(\s+[^>]*?)?\s+class\s*=\s*"Apple-style-span"(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/gi, d), a = a.replace(/<(span)(\s+[^>]*?)?\s+class\s*=\s*"Apple-style-span"(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?)<\/\1>/gi, d), a = a.replace(/<(span)(\s+[^>]*?)?\s+class\s*=\s*"Apple-style-span"(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S])*?)<\/\1>/gi, d);
					a = a.replace(/(<(\w+))((?:\s+[\w\-:]+\s*=\s*(?:"[^"]*"|'[^']*'|[^>\s]+))*)\s*(\/?>)/g, function(a, b, c, d, e) {
						c = c.toLowerCase();
						var g;
						return d = d.replace(/\s+_xhe_(?:src|href)\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/i, function(a, b) {
							return g = b.match(/^(["']?)(.*)\1/)[2], ""
						}), g && urlType && (g = getLocalUrl(g, urlType, urlBase)), d = d.replace(/\s+([\w\-:]+)\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/g, function(a, b, c) {
							if (b = b.toLowerCase(), c = c.match(/^(["']?)(.*)\1/)[2].replace(/"/g, "'"), "class" === b) {
								...
								"style" === b && (c = c.replace(/(^|;)\s*(font-size)\s*:\s*([a-z-]+)\s*(;|$)/i, function(a, b, c, d, e) {
							...
							return " " + b + '="' + c + '"'
						}), "img" !== c || d.match(/\s+alt\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/i) || (d += ' alt=""'), b + d + e
					}), a = a.replace(/(<(td|th)(?:\s+[^>]*?)?>)\s*([\s\S]*?)(<br(\s*\/)?>)?\s*<\/\2>/gi, function(a, b, c, d) {
						return b + (d ? d : "&nbsp;") + "</" + c + ">"
					}), a = a.replace(/^\s*(?:<(p|div)(?:\s+[^>]*?)?>)?\s*(<span(?:\s+[^>]*?)?>\s*<\/span>|<br(?:\s+[^>]*?)?>|&nbsp;)*\s*(?:<\/\1>)?\s*$/i, "")
				}
				return a = a.replace(/(<pre(?:\s+[^>]*?)?>)([\s\S]+?)(<\/pre>)/gi, function(a, b, c, d) {
					return b + c.replace(/<br\s*\/?>/gi, "\r\n") + d
				})
			}, this.getSource = function (a) {
				
				var b, c = settings.beforeGetSource;
				return bSource ? (b = $("#sourceCode", _doc).val(), c || (b = _this.formatXHTML(b, !1))) : (b = _this.processHTML(_doc.body.innerHTML, "read"), b = _this.cleanHTML(b), b = _this.formatXHTML(b, a), c && (b = c(b))), _text.value = b, b
				...
				if (c > 0 && 3 > c && /mso(-|normal)|WordDocument|<table\s+[^>]*?x:str|\s+class\s*=\s*"?xl[67]\d"/i.test(a)) {
					a = a.replace(/<!--[\s\S]*?-->|<!(--)?\[[\s\S]+?\](--)?>|<style(\s+[^>]*?)?>[\s\S]*?<\/style>/gi, ""), a = a.replace(/\r?\n/gi, ""), isIE ? (a = a.replace(/<v:shapetype(\s+[^>]*)?>[\s\S]*<\/v:shapetype>/gi, ""), a = a.replace(/<v:shape(\s+[^>]+)?>[\s\S]*?<v:imagedata(\s+[^>]+)?>\s*<\/v:imagedata>[\s\S]*?<\/v:shape>/gi, function(a, b, c) {
						var d;
						if (d = c.match(/\s+src\s*=\s*("[^"]+"|'[^']+'|[^>\s]+)/i)) {
							d = d[1].match(/^(["']?)(.*)\1/)[2];
							var e = '<img src="' + editorRoot + 'xheditor_skin/blank.gif" _xhe_temp="true" class="wordImage"';
							return d = b.match(/\s+style\s*=\s*("[^"]+"|'[^']+'|[^>\s]+)/i), d && (d = d[1].match(/^(["']?)(.*)\1/)[2], e += ' style="' + d + '"'), e += " />"
						}
						return ""
					})) : a = a.replace(/<img( [^<>]*(v:shapes|msohtmlclip)[^<>]*)\/?>/gi, function(a, b) {
						var c, d = '<img src="' + editorRoot + 'xheditor_skin/blank.gif" _xhe_temp="true" class="wordImage"';
						return c = b.match(/ width\s*=\s*"([^"]+)"/i), c && (d += ' width="' + c[1] + '"'), c = b.match(/ height\s*=\s*"([^"]+)"/i), c && (d += ' height="' + c[1] + '"'), d + " />"
					}), a = a.replace(/(<(\/?)([\w\-:]+))((?:\s+[\w\-:]+(?:\s*=\s*(?:"[^"]*"|'[^']*'|[^>\s]+))?)*)\s*(\/?>)/g, function(a, b, d, e, f, g) {
						return e = e.toLowerCase(), e.match(/^(link)$/) && f.match(/file:\/\//i) || e.match(/:/) || "span" === e && 2 === c ? "" : (d || (f = f.replace(/\s([\w\-:]+)(?:\s*=\s*("[^"]*"|'[^']*'|[^>\s]+))?/gi, function(a, b, d) {
							if (b = b.toLowerCase(), /:/.test(b)) return "";
							if (d = d.match(/^(["']?)(.*)\1/)[2], 1 === c) switch (e) {
							case "p":
								if ("style" === b) return d = d.replace(/"|&quot;/gi, "'").replace(/\s*([^:]+)\s*:\s*(.*?)(;|$)/gi, function(a, b, c) {
					...
					for (var d = 0; 3 > d; d++) a = a.replace(/<font(\s+[^>]+)><font(\s+[^>]+)>/gi, function(a, b, c) {
						return "<font" + b + c + ">"
					});
					a = a.replace(/(<(\/?)(tr|td)(?:\s+[^>]+)?>)[^<>]+/gi, function(a, b, c, d) {
				...
				var b;
				return a = a.replace(/<head(?:\s+[^>]*?)?>([\s\S]*?)<\/head>/i, function(a, c) {
					return b = c.match(/<(script|style)(\s+[^>]*?)?>[\s\S]*?<\/\1>/gi), ""
				}), b && (a = b.join("") + a), a = a.replace(/<\??xml(:\w+)?(\s+[^>]*?)?>([\s\S]*?<\/xml>)?/gi, ""), settings.internalScript || (a = a.replace(/<script(\s+[^>]*?)?>[\s\S]*?<\/script>/gi, "")), settings.internalStyle || (a = a.replace(/<style(\s+[^>]*?)?>[\s\S]*?<\/style>/gi, "")), settings.linkTag && settings.inlineScript && settings.inlineStyle || (a = a.replace(/(<(\w+))((?:\s+[\w-]+\s*=\s*(?:"[^"]*"|'[^']*'|[^>\s]+))*)\s*(\/?>)/gi, function(a, b, c, d, e) {
				...
				function j(a, c, d) {
					if (H || (a = a.replace(/(\t*\r?\n\t*)+/g, "")), H || b !== !0) v.push(a);
						...
						var e = n[c],
							f = e ? c : "";
						e ? (d && F++, "" === G && F--) : G && F++, (f !== G || e) && k(), v.push(a), "br" === c && k(), !e || !m[c] && d || F--, G = e ? c : "", D = d
					}
				}
				function k() {
				...
				function l(a, b, c, d) {
					if (!c) return d;
					var e = "";
					return c = c.replace(/ face\s*=\s*"\s*([^"]*)\s*"/i, function(a, b) {
				...
				for (var x, y, z, A, B, C, D, E = 0, F = -1, G = "body", H = !1; x = t.exec(a);) y = x.index, y > E && (C = a.substring(E, y), A ? B.push(C) : g(C)), E = t.lastIndex, !(z = x[1]) || (z = d(z), A && z === A && (h(B.join("")), A = null, B = null), A) ? A ? B.push(x[0]) : (z = x[3]) ? (z = d(z), e(z, x[4], x[5]), r[z] && (A = z, B = [])) : x[2] && i(x[0]) : f(z);
				a.length > E && g(a.substring(E, a.length)), f(), a = v.join(""), v = null;
				var I = settings.listFontsize;
				return a = a.replace(/<(font)(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/gi, l), a = a.replace(/<(font)(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?)<\/\1>/gi, l), a = a.replace(/<(font)(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S])*?)<\/\1>/gi, l), a = a.replace(/^(\s*\r?\n)+|(\s*\r?\n)+$/g, "")
				...
				if (bSource !== a) {
					...
					if (bSource ? (b = _this.getSource(), e.html("").removeAttr("scroll").attr("class", "editMode" + bodyClass), isIE ? d.contentEditable = "true" : _doc.designMode = "On", isMozilla && (_this._exec("inserthtml", "-"), $("#" + idFixFFCursor).show().focus().hide()), h = "Source") : (_this.pasteHTML(f, !0), b = _this.getSource(!0), g = b.indexOf(f), isOpera || (g = b.substring(0, g).replace(/\r/g, "").length), b = b.replace(/(\r?\n\s*|)<span id="_xhe_cursor"><\/span>(\s*\r?\n|)/, function(a, b, c) {
						...
						i.move("character", g), i.select()
					} else _this.setTextCursor();
					_jTools.find("[cmd=Source]").attr("title", getLang(h)).find("span").text(h), _jTools.find("[cmd=Source],[cmd=Preview]").toggleClass("xheEnabled"), _jTools.find(".xheButton").not("[cmd=Source],[cmd=Fullscreen],[cmd=About]").toggleClass("xheEnabled").attr("aria-disabled", bSource ? !0 : !1), setTimeout(setOpts, 300)
				}
				...
				var c = "<html><head>" + headHTML + "<title>{#Preview}</title>" + (urlBase ? '<base href="' + urlBase + '"/>' : "") + "</head><body>" + b + "</body></html>",
					...
					f = e.document;
				f.open(), f.write(getLang(c)), f.close(), e.focus()
				...
				}), c.append(getLang(e.join(""))), c.click(function(a) {
					return a = a.target, $.nodeName(a, "DIV") ? void 0 : (_this.loadBookmark(), b($(a).closest("a").attr("v")), _this.hidePanel(), !1)
				}).mousedown(returnFalse), _this.saveBookmark(), _this.showPanel(c)
				...
				var b = settings.listColors,
					...
					f = 0;
				$.each(b, function(a, b) {
					f % 7 === 0 && d.push((f > 0 ? "</div>" : "") + "<div>"), d.push("<a href=\"javascript:void('" + b + '\')" xhev="' + b + '" title="' + b + '" style="background:' + b + '" role="option" aria-setsize="' + e + '" aria-posinset="' + (f + 1) + '"></a>'), f++
				}), d.push("</div>"), c.append(d.join("")), c.click(function(b) {
					return b = b.target, $.nodeName(b, "A") ? (_this.loadBookmark(), a($(b).attr("xhev")), _this.hidePanel(), !1) : void 0
				}).mousedown(returnFalse), _this.saveBookmark(), _this.showPanel(c)
				...
				c.click(function() {
					_this.loadBookmark();
					var a = b.val();
					return a && _this.pasteText(a), _this.hidePanel(), !1
				...
				if (c && e.find("#xheLinkAnchor").change(function() {
					...
					if (!f.attr("href")) return ev = null, _this.exec("Anchor");
					h.val(xheAttr(f, "href")), i.attr("value", f.attr("target"))
				} else "" === k && g.val(getLang(settings.defLinkText)).closest("div").show();
				settings.upLinkUrl && _this.uploadInit(h, settings.upLinkUrl, settings.upLinkExt), j.click(function() {
					_this.loadBookmark();
					var a = h.val();
					if (("" === a || 0 === f.length) && _this._exec("unlink"), "" !== a && "http://" !== a) {
						var b = a.split(" "),
							...
							d = g.val();
						if (b.length > 1) {
							_this._exec("unlink"), k = _this.getSelect();
							var e, j = '<a href="xhe_tmpurl"',
								l = [];
							"" !== c && (j += ' target="' + c + '"'), j += ">xhe_tmptext</a>", d = "" !== k ? k : d ? d : a;
							for (var m = 0, n = b.length; n > m; m++) a = b[m], "" !== a && (a = a.split("||"), e = j, e = e.replace("xhe_tmpurl", a[0]), e = e.replace("xhe_tmptext", a[1] ? a[1] : d), l.push(e));
							_this.pasteHTML(l.join("&nbsp;"))
						} else a = b[0].split("||"), d || (d = a[0]), d = a[1] ? a[1] : "" !== k ? "" : d ? d : a[0], 0 === f.length ? (d ? _this.pasteHTML('<a href="#xhe_tmpurl">' + d + "</a>") : _this._exec("createlink", "#xhe_tmpurl"), f = $('a[href$="#xhe_tmpurl"]', _doc)) : d && !isWebkit && f.text(d), xheAttr(f, "href", a[0]), "" !== c ? f.attr("target", c) : f.removeAttr("target")
					}
				...
				var a = $(getLang(htmlAnchor)),
					...
					d = $("#xheSave", a);
				if (1 === b.length) {
					if (b.attr("href")) return ev = null, _this.exec("Link");
					c.val(b.attr("name"))
				}
				d.click(function() {
					_this.loadBookmark();
					var a = c.val();
					return a ? 0 === b.length ? _this.pasteHTML('<a name="' + a + '"></a>') : b.attr("name", a) : 1 === b.length && b.remove(), _this.hidePanel(), !1
				}), _this.saveBookmark(), _this.showDialog(a)
				...
				var a = $(getLang(htmlImg)),
					...
					k = $("#xheSave", a);
				if (1 === b.length) {
					c.val(xheAttr(b, "src")), d.val(b.attr("alt")), e.val(b.attr("align")), f.val(b.attr("width")), g.val(b.attr("height")), h.val(b.attr("border"));
					var l = b.attr("vspace"),
						m = b.attr("hspace");
					i.val(0 >= l ? "" : l), j.val(0 >= m ? "" : m)
				}
				settings.upImgUrl && _this.uploadInit(c, settings.upImgUrl, settings.upImgExt), k.click(function() {
					_this.loadBookmark();
					var a = c.val();
					if ("" !== a && "http://" !== a) {
						var k = a.split(" "),
						...
						if (k.length > 1) {
							var s, t = '<img src="xhe_tmpurl"',
								u = [];
							"" !== l && (t += ' alt="' + l + '"'), "" !== m && (t += ' align="' + m + '"'), "" !== n && (t += ' width="' + n + '"'), "" !== o && (t += ' height="' + o + '"'), "" !== p && (t += ' border="' + p + '"'), "" !== q && (t += ' vspace="' + q + '"'), "" !== r && (t += ' hspace="' + r + '"'), t += " />";
							for (var v in k) a = k[v], "" !== a && (a = a.split("||"), s = t, s = s.replace("xhe_tmpurl", a[0]), a[1] && (s = '<a href="' + a[1] + '" target="_blank">' + s + "</a>"), u.push(s));
							...
							var w = b.parent("a");
							0 === w.length && (b.wrap("<a></a>"), w = b.parent("a")), xheAttr(w, "href", a[1]), w.attr("target", "_blank")
						}
				...
				var h = $(getLang(b)),
					...
					l = $("#xhe" + a + "Height", h),
					m = $("#xheSave", h);
				f && _this.uploadInit(j, f, g), 1 === i.length && (j.val(xheAttr(i, "src")), k.val(i.attr("width")), l.val(i.attr("height"))), m.click(function() {
					_this.loadBookmark();
					var a = j.val();
					if ("" !== a && "http://" !== a) {
						var b = k.val(),
						...
						if (m.length > 1) {
							...
							for (var q in m) a = m[q].split("||"), n = o, n = n.replace("xhe_tmpurl", a[0]), n = n.replace("xhe_width", a[1] ? a[1] : b), n = n.replace("xhe_height", a[2] ? a[2] : f), "" !== a && p.push(n);
					...
					return _this.hidePanel(), !1
				}), _this.saveBookmark(), _this.showDialog(h)
				...
				var c = arrEmots[a],
					...
					k = c.count,
					l = c.list;
				if (k) for (var m = 1; k >= m; m++) e++, f.push("<a href=\"javascript:void('" + m + '\')" style="background-image:url(' + d + m + '.gif);" emot="' + a + "," + m + '" xhev="" title="' + m + '" role="option">&nbsp;</a>'), e % j === 0 && f.push("<br />");
				else {
					var n = is(l, "array");
					$.each(l, function(b, c) {
						n && (b = c, c = "{#emot." + a + "." + b + "}"), e++, f.push("<a href=\"javascript:void('" + c + '\')" style="background-image:url(' + d + b + '.gif);" emot="' + a + "," + b + '" title="' + c + '" xhev="' + c + '" role="option">&nbsp;</a>'), e % j === 0 && f.push("<br />")
					})
				}
				var o = j * (h + 12),
					p = Math.ceil(e / j) * (i + 12),
					q = .75 * o;
				q >= p && (q = ""), g = $(getLang("<style>" + (q ? ".xheEmot div{width:" + (o + 20) + "px;height:" + q + "px;}" : "") + ".xheEmot div a{width:" + h + "px;height:" + i + "px;}</style><div>" + f.join("") + "</div>")).click(function(a) {
					a = a.target;
					var b = $(a);
					if ($.nodeName(a, "A")) return _this.loadBookmark(), _this.pasteHTML('<img emot="' + b.attr("emot") + '" alt="' + b.attr("xhev") + '">'), _this.hidePanel(), !1
				}).mousedown(returnFalse), b.append(g);
				var r, s = 0,
					t = ['<ul role="tablist">'];
				$.each(arrEmots, function(b, c) {
					s++, t.push("<li" + (a === b ? ' class="cur"' : "") + ' role="presentation"><a href="javascript:void(\'' + c.name + '\')" group="' + b + '" role="tab" tabindex="0">' + c.name + "</a></li>")
				}), s > 1 && (t.push('</ul><br style="clear:both;" />'), r = $(getLang(t.join(""))).click(function(a) {
					return selEmotGroup = $(a.target).attr("group"), _this.exec("Emot"), !1
				}).mousedown(returnFalse), b.append(r)), _this.saveBookmark(), _this.showPanel(b)
				...
				l.click(function() {
					...
					return w += "</tbody></table>", _this.pasteHTML(w), _this.hidePanel(), !1
				}), _this.saveBookmark(), _this.showDialog(a)
			}, this.showAbout = function() {
				var a = $(getLang(htmlAbout));
				a.find("p").attr("role", "presentation"), _this.showDialog(a, !0), setTimeout(function() {
					a.focus()
				}, 100)
			}, this.addShortcuts = function(a, b) {
				a = a.toLowerCase(), arrShortCuts[a] === undefined && (arrShortCuts[a] = Array()), arrShortCuts[a].push(b)
				...
				function d(b) {
					...
					var c, d, e, f = !1,
						...
						h = [],
						i = settings.onUpload;
					for (i && i(b), c = 0; g > c; c++) d = b[c], e = is(d, "string") ? d : d.url, "!" === e.substr(0, 1) && (f = !0, e = e.substr(1)), h.push(e);
					a.val(h.join(" ")), f && a.closest(".xheDialog").find("#xheSave").click()
				}
				...
				if (a.after(e), f.before(a), b = b.replace(/{editorRoot}/gi, editorRoot), "!" === b.substr(0, 1)) f.click(function() {
					...
					var i, j = $(".xheFile", e);
					j.change(function() {
						i = [], _this.startUpload(j[0], b, c, d)
					}), setTimeout(function() {
						a.closest(".xheDialog").bind("dragenter dragover", returnFalse).bind("drop", function(a) {
							var e, f = a.originalEvent.dataTransfer;
							return g && f && (e = f.files) && e.length > 0 && _this.startUpload(e, b, c, d), !1
						})
					}, 10)
				}
				...
				var arrMsg = [],
					...
					upload, fileList, filename, jUploadTip = $(getLang('<div style="padding:22px 0;text-align:center;line-height:30px;">{#upload.progressTip}<br /></div>')),
				...
				if (!isOpera && bHtml5Upload && (!fromFiles.nodeType || (fileList = fromFiles.files) && fileList[0].name)) {
					...
					jUploadTip.append(jProgress), upload = new _this.html5Upload(uploadInputname, fileList, toUrl, onUploadCallback, function(a) {
						if (a.loaded >= 0) {
							var b = Math.round(100 * a.loaded / a.total) + "%";
							$("div", jProgress).css("width", b), $("span", jProgress).text(b + " ( " + formatBytes(a.loaded) + " / " + formatBytes(a.total) + " )")
				...
				return i.before(j).appendTo(h), this.remove = function() {
					null !== f && (j.before(i).remove(), g.remove(), h.remove(), f = null)
				}, this.onLoad = function() {
					var a = g[0].contentWindow.document,
						b = $(a.body).text();
					a.write(""), f.remove(), c(b, !0)
				}, this.start = function() {
					h.submit(), g.load(f.onLoad)
				}, this
			}, this.html5Upload = function(a, b, c, d, e) {
				function f(b, c, d, e) {
					h = new XMLHttpRequest, upload = h.upload, h.onreadystatechange = function() {
						4 === h.readyState && d(h.responseText)
					}, upload ? upload.onprogress = function(a) {
						e(a.loaded)
					} : e(-1), h.open("POST", c), h.setRequestHeader("Content-Type", "application/octet-stream"), h.setRequestHeader("Content-Disposition", 'attachment; name="' + encodeURIComponent(a) + '"; filename="' + encodeURIComponent(b.name) + '"'), h.sendAsBinary && b.getAsBinary ? h.sendAsBinary(b.getAsBinary()) : h.send(b)
				}
				function g(a) {
					e && e({
						loaded: k + a,
				...
				for (var h, i = 0, j = b.length, k = 0, l = 0, m = this, n = 0; j > n; n++) l += b[n].size;
				this.remove = function() {
					h && (h.abort(), h = null)
				}, this.uploadNext = function(a) {
					a && (k += b[i - 1].size, g(0)), (!a || a && d(a, i === j) === !0) && j > i && f(b[i++], c, m.uploadNext, function(a) {
						g(a)
					})
				}, this.start = function() {
					m.uploadNext()
				...
				function initModalWin() {
					try {
						modalWin.callback = callbackModal, modalWin.unloadme = _this.removeModal, $(modalWin.document).keydown(checkEsc), result = modalWin.name
					} catch (a) {}
				}
				function callbackModal(a) {
					modalWin.document.write(""), _this.removeModal(), null != a && callback(a)
				}
				...
				var modalWin = jIframe[0].contentWindow,
					result;
				initModalWin(), jIframe.load(function() {
				...
				c.append(d), _this.showPanel(c, b)
			}, this.showPanel = function(a, b) {
				if (!ev.target) return !1;
				_jPanel.html("").append(a).css("left", -999).css("top", -999), _jPanelButton = $(ev.target).closest("a").addClass("xheActive");
				var c = _jPanelButton.offset(),
					d = c.left,
					e = c.top;
				e += _jPanelButton.outerHeight() - 1, _jCntLine.css({
					left: d + 1,
					top: e,
				...
				i && !isNaN(i) && (_jShadow.css("zIndex", parseInt(i, 10) + 1), _jPanel.css("zIndex", parseInt(i, 10) + 2), _jCntLine.css("zIndex", parseInt(i, 10) + 3)), _jPanel.css({
					left: d,
					top: e
				}).show(), b || setTimeout(function() {
				...
				case "blocktag":
					var e = [];
					$.each(settings.listBlocktag, function(a, b) {
						e.push({
							s: "<" + b.n + ">{#listBlocktag." + b.n + "}</" + b.n + ">",
							v: "<" + b.n + ">",
							t: "{#listBlocktag." + b.n + "}"
						})
					}), _this.showMenu(e, function(a) {
						_this._exec("formatblock", a)
					});
					break;
				case "fontface":
					var f = [];
					$.each(getLang("listFontname"), function(a, b) {
						b.c = b.c ? b.c : b.n, f.push({
							s: '<span style="font-family:' + b.c + '">' + b.n + "</span>",
							v: b.c,
							t: b.n
						})
					}), _this.showMenu(f, function(a) {
						_this._exec("fontname", a)
					});
					break;
				case "fontsize":
					var g = [];
					$.each(settings.listFontsize, function(a, b) {
						g.push({
							s: '<span style="font-size:' + b.s + ';">{#fontsize.' + b.n + "}(" + b.s + ")</span>",
							v: a + 1,
							t: "{#fontsize." + b.n + "}"
						})
					}), _this.showMenu(g, function(a) {
				...
				case "align":
					var h = [];
					$.each(arrAlign, function(a, b) {
						h.push({
							s: "{#align." + b.v + "}",
							v: b.v
						})
					}), _this.showMenu(h, function(a) {
						_this._exec(a)
					});
					break;
				case "list":
					var i = [];
					$.each(arrList, function(a, b) {
						i.push({
							s: "{#list." + b.v + "}",
							v: b.v
						})
					}), _this.showMenu(i, function(a) {
						_this._exec(a)
					});
					break;
				case "link":
	...
	}, $.fn.oldVal = $.fn.val, $.fn.val = function(a) {
		var b, c = this;
